...
// Obtener los departamentos (catálogos) directamente de la base de datos
try {
    $stmt = $pdo->prepare("SELECT id, nombre, imagen_url FROM catalogos WHERE estatus = 'activo' ORDER BY nombre ASC");
    $stmt->execute();
    $departamentos = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $departamentos = []; // En caso de error, el array estará vacío
}
?>
        ...
        <?php if (!empty($departamentos)): ?>
            <?php foreach ($departamentos as $depto): ?>
                <div class="col-6 col-md-4 col-lg-2">
                    <a href="tienda.php?categoria=<?php echo htmlspecialchars($depto['id']); ?>" class="text-decoration-none department-item">
                        <?php
                        $imagen = !empty($depto['imagen_url']) ? ltrim($depto['imagen_url'], '/') : 'front/multimedia/default.png';
                        ?>
                        <img src="<?php echo htmlspecialchars($imagen); ?>" alt="<?php echo htmlspecialchars($depto['nombre']); ?>">
                        <p><?php echo htmlspecialchars($depto['nombre']); ?></p>
                    </a>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
