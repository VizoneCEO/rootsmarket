        ...
        switch ($action) {
            ...
            case 'create':
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    // --- SUBIDA DE IMÁGENES CORREGIDA (NOMBRES LIMPIOS) ---
                    if (isset($_FILES['imagenes']) && !empty(array_filter($_FILES['imagenes']['name']))) {
                        ...
                        foreach ($_FILES['imagenes']['name'] as $key => $name) {
                            if ($_FILES['imagenes']['error'][$key] == 0) {
                                // 1. Obtener extensión segura
                                $ext = strtolower(pathinfo($name, PATHINFO_EXTENSION));

                                // 2. Generar nombre limpio (ID único + extensión)
                                // Ejemplo resultante: "65a1b2c3d4e5f.png"
